...
class EIAv2Client:
    ...
    def fetch_hourly_grid_data(
        ...
        """
        Extracts hourly demand, solar generation, and wind generation.
        """
        if not self.api_key or self.api_key == "your_actual_eia_v2_api_key_here":
        ...
        logging.info(f"Querying EIA v2 API for region: {respondent}...")
        ...
        try:
            ...
            if not records:
                logging.warning("EIA API returned empty response payload. Switching to fallback generator.")
                return self._generate_fallback_eia_data(start_date, end_date)

            df = pd.DataFrame(records)
            
            # Pivot EIA response to have distinct columns per metric type
            # (D = Demand, SUN = Solar, WND = Wind)
            ...
            logging.error(f"Failed to fetch EIA v2 grid data: {e}. Generating fallback dataset.")
            return self._generate_fallback_eia_data(start_date, end_date)

    def _generate_fallback_eia_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        """Generates realistic synthetic EIA grid data if API key is missing or invalid."""
        logging.info("Generating synthetic fallback EIA operational data...")
